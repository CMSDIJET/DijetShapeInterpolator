...
for(int alphabin=0; alphabin<=13; alphabin++)
{
	...
	for(int Suu=2000; Suu<=11000; Suu=Suu+100)
	{
		...
		if(alphabin!=0) sprintf(output_filename,"/eos/cms/store/group/phys_exotica/dijet/Dijet13TeV/ilias/4jets_Run2/Wide_Resonance_Search/signal_histos/alpha%s/ResonanceShapes_%s_Suu-Diquark_W-%s_S-%d_chi-%.0f_alphabin%d.root", alpha_true_name, type, width_name, Suu, Suu*alpha_true, alphabin);
		if(alphabin==0) sprintf(output_filename,"/eos/cms/store/group/phys_exotica/dijet/Dijet13TeV/ilias/4jets_Run2/Wide_Resonance_Search/signal_histos/alpha%s/ResonanceShapes_%s_Suu-Diquark_W-%s_S-%d_chi-%.0f_alphagt0p1.root", alpha_true_name, type, width_name, Suu, Suu*alpha_true);
